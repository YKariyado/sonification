...
def midi_to_samples(file_name, encode_length=False, num_notes=96, samples_per_measure=96):
    ...
    # time signatureのメッセージを受け取って複数個ある場合はNotImplementedErrorを返す。
    # 仕組みとしてはnew_tpmが更新される（リズム遷移）場合、has_time_sigフラグがTrueでさらにtick_per_measureとズレてる判定になるから上のエラー処理が出る
    ...
    for track in mid.tracks:
        ...
        abs_time = 0
        for msg in track:
            abs_time += msg.time  # step time forward (absolute time in "tick")
            ...
            # if a note ends
                ...
                # len(all_notes[54][-1]) = 1 つまり 54: [[0.77777, 0.80000] [0.90000,ここに下の計算したtickが入る]]
